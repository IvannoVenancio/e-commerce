...
modelsJson.map((item, index)=>{
    ...
    modelsItem.querySelector('.models-item--price').innerHTML = `Kz ${item.price[0].toFixed(2)}`;
...
function updateCart() {
    ...
    if(cart.length > 0) {
        ...
        c('.subtotal span:last-child').innerHTML = `Kz ${subtotal.toFixed(2)}`;
        c('.desconto span:last-child').innerHTML = `Kz ${desconto.toFixed(2)}`;
        c('.total span:last-child').innerHTML = `Kz ${total.toFixed(2)}`;
    } else {
